...
from streamlit_js_eval import streamlit_js_eval # <-- Import component
        ...
        for item in data['study'][part_key]:
            if 'video_path' in item and os.path.exists(item['video_path']):
                item['orientation'] = get_video_orientation(item['video_path'])
                 ...
                 st.warning(f"Warning: Study video file not found at '{item['video_path']}'. Defaulting orientation.")
    ...
    for part_key in data['quiz']:
         for item in data['quiz'][part_key]:
            if 'video_path' in item and os.path.exists(item['video_path']):
                ...
                st.warning(f"Warning: Quiz video file not found at '{item['video_path']}'. Defaulting orientation.")
                item['orientation'] = 'landscape'
            else: item['orientation'] = 'landscape'
    return data
...
/* Keyframes for the new caption highlight */
@keyframes highlight-new {
    0% { border-color: transparent; box-shadow: none; }
    25% { border-color: #facc15; box-shadow: 0 0 8px #facc15; } /* Yellow glow */
    75% { border-color: #facc15; box-shadow: 0 0 8px #facc15; } /* Hold glow */
    100% { border-color: transparent; box-shadow: none; }
}
/* Base style for caption box */
...
# --- CENTRAL DICTIONARY ---
DEFINITIONS = { 'Adventurous': '...', 'Amusing': '...', 'Angry': '...', 'Anxious': '...', 'Appreciative': '...', 'Assertive': '...', 'Caring': '...', 'Considerate': '...', 'Critical': '...', 'Cynical (Doubtful, Skeptical)': '...', 'Emotional': '...', 'Energetic': '...', 'Enthusiastic': '...', 'Observant': '...', 'Objective (Detached, Impartial)': '...', 'Questioning': '...', 'Reflective': '...', 'Sarcastic': '...', 'Serious': '...', 'Advisory': '...', 'CallToAction': '...', 'Conversational': '...', 'Exaggeration': '...', 'Factual': '...', 'Instructional': '...', 'Judgmental': '...', 'Metaphorical': '...', 'Persuasive': '...', 'Rhetorical Question': '...', 'Public Safety Alert': '...', 'Social Media Update': '...', 'Driver Behavior Monitoring': '...', 'Law Enforcement Alert': '...', 'Traffic Analysis': '...', 'Community Road Safety Awareness': '...', 'Public Safety Awareness': '...', 'Road Safety Education': '...', 'Traffic Awareness': '...' }
        ...
        if current_step >= 4:
            if st.session_state.show_feedback:
                ...
                with st.form("quiz_form"):
                    choice = [opt for opt in question_data['options'] if st.checkbox(opt, key=f"cb_{current_index}_{opt}")] if question_data.get("question_type") == "multi" else st.radio("Select one option:", question_data['options'], key=f"radio_{current_index}", index=None, format_func=format_options_with_info)
        ...
        with col1:
            ...
            if current_step >= 2:
                ...
                if st.session_state.get(summary_typed_key, False): st.info(current_comp["video_summary"])
                    ...
                    st.session_state[summary_typed_key] = True
                if current_step == 2:
                    if st.button("Proceed to Captions", key=f"p2_proceed_captions_{comparison_id}"): st.session_state[view_state_key]['step'] = 3; st.rerun()
        with col2:
            if current_step >= 3:
            ...
            if current_step >= 4:
                ...
                personality_traits = list(control_scores.get("personality", {}).keys()); style_traits = list(control_scores.get("writing_style", {}).keys())
        ...
        with col1:
            ...
            if current_step >= 2:
                ...
                if st.session_state.get(summary_typed_key, False): st.info(current_change["video_summary"])
                    ...
                    st.session_state[summary_typed_key] = True
                if current_step == 2:
                    if st.button("Proceed to Captions", key=f"p3_proceed_captions_{change_id}"): st.session_state[view_state_key]['step'] = 3; st.rerun()
        with col2:
            if current_step >= 3:
                ...
                if current_step == 3:
                     if st.button("Show Questions", key=f"p3_show_q_{change_id}"): st.session_state[view_state_key]['step'] = 4; st.rerun()
            if current_step >= 4:
                trait = field_to_change[field_type]; terms_to_define.add(trait)
                with st.form(key=f"study_form_change_{change_idx}"):
                    q_template_key = field_type.replace('_', ' ').title(); q_template = st.session_state.all_data['questions']['part3_questions'][q_template_key]
                    format_args = {'change_type': current_change['change_type']}
                    if field_type == 'personality': format_args['Personality'] = f"<b class='highlight-trait'>{trait}</b>"; format_args['Writing Style'] = ""
                    elif field_type == 'writing_style': format_args['Writing Style'] = f"<b class='highlight-trait'>{trait}</b>"; format_args['Personality'] = ""
                    dynamic_question_raw = q_template.format(**format_args); dynamic_question_save = re.sub('<[^<]+?>', '', dynamic_question_raw)
