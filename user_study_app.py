...
# --- ADDED: Function to skip to questions ---
def skip_to_questions(timer_key, summary_key, view_key):
    """Sets state to skip video/summary and go directly to questions."""
    st.session_state[timer_key] = True
    if summary_key:
        st.session_state[summary_key] = True
    # Ensure view state exists before modifying
    if view_key not in st.session_state:
         # Initialize with step 6 if it doesn't exist
         # (Need to adapt based on how view_state is structured for each part)
         if 'p1' in view_key:
              q_templates = st.session_state.all_data['questions']['part1_questions']
              question_ids = [q['id'] for q in q_templates if q['id'] != 'overall_relevance']
              st.session_state[view_key] = {'step': 6, 'interacted': {qid: False for qid in question_ids}, 'comp_feedback': False, 'comp_choice': None}
         elif 'p2' in view_key:
              q_templates = st.session_state.all_data['questions']['part2_questions']
              question_ids = [q['id'] for q in q_templates]
              st.session_state[view_key] = {'step': 6, 'interacted': {qid: False for qid in question_ids}, 'comp_feedback': False, 'comp_choice': None}
         elif 'p3' in view_key:
              st.session_state[view_key] = {'step': 6, 'summary_typed': True, 'comp_feedback': False, 'comp_choice': None} # Part 3 structure is slightly different
    else:
...
if st.session_state.page == 'demographics':
    ...
    if st.session_state.study_part == 1:
        ...
        # --- Define keys needed for skip button ---
        current_caption = current_video['captions'][caption_idx] # Need caption for view_state_key
        view_state_key = f"view_state_p1_{current_caption['caption_id']}"
        summary_typed_key = f"summary_typed_{current_video['video_id']}" if caption_idx == 0 else None # Summary only for first caption

        # --- Initial video display or skip button logic ---
        if not st.session_state.get(timer_finished_key, False) and caption_idx == 0:
            head_cols = st.columns([3, 1]) # Add columns for button
            with head_cols[0]:
                st.subheader("Watch the video")
            with head_cols[1]:
                # --- Skip Button Added ---
                st.button("Skip to Questions", key=f"skip_p1_{video_id}",
                          on_click=skip_to_questions, args=(timer_finished_key, summary_typed_key, view_state_key))
            with st.spinner(""):
            ...
            st.session_state[timer_finished_key] = True
            st.rerun()
        else: # Normal flow after video or if skipping
            q_templates = st.session_state.all_data['questions']['part1_questions']
            ...
            # Initialize view state if needed (might be initialized by skip button)
            if view_state_key not in st.session_state:
                ...
                if caption_idx == 0 and summary_typed_key and summary_typed_key not in st.session_state:
            ...
            with title_col1:
                st.subheader("Video")
            with title_col2:
                if current_step >= 5: # Only show title if past comprehension/summary
            ...
            with col1:
                ...
                if caption_idx == 0:
                    ...
                    elif current_step >= 2 and current_step < 6: # Show summary only during steps 2-5
                        st.subheader("Video Summary")
                ...
                elif current_step < 6: # Show summary for subsequent captions if before questions
            ...
            with col2:
                ...
                terms_to_define = set()
                # Show caption box from step 5 onwards
                if current_step >= 5:
                ...
                # Show questions from step 6 onwards
                if current_step >= 6:
        ...
        if not st.session_state.get(timer_finished_key, False):
            ...
            with head_cols[1]:
                 # --- Skip Button Added ---
                 st.button("Skip to Questions", key=f"skip_p2_{comparison_id}",
                           on_click=skip_to_questions, args=(timer_finished_key, summary_typed_key, view_state_key))
            with st.spinner(""):
                main_col, _ = st.columns([1, 1.8])
                with main_col:
            ...
            if view_state_key not in st.session_state:
                ...
                if summary_typed_key not in st.session_state:
                    st.session_state[summary_typed_key] = False
            ...
            with col1:
                ...
                if current_step == 1:
                    if st.button("Proceed to Summary", key=f"p2_proceed_summary_{comparison_id}"):
                        st.session_state[view_state_key]['step'] = 2; st.rerun()
                if current_step >= 2 and current_step < 6: # Show summary only during steps 2-5
                    st.subheader("Video Summary")
            ...
            with col2:
                ...
                validation_placeholder = st.empty()
                terms_to_define = set()
                # Show captions from step 5 onwards
                if current_step >= 5:
        ...
        if not st.session_state.get(timer_finished_key, False):
            ...
            with head_cols[1]:
                 # --- Skip Button Added ---
                 st.button("Skip to Questions", key=f"skip_p3_{change_id}",
                           on_click=skip_to_questions, args=(timer_finished_key, summary_typed_key, view_state_key))
            with st.spinner(""):
                main_col, _ = st.columns([1, 1.8])
                with main_col:
...
parent_document.addEventListener('keyup', function(event) {
    ...
    if (event.key === 'ArrowRight') {
        ...
        const targetButtonLabels = [
            ...
            "Skip to Questions" // Added Skip button
        ];
...
"""
streamlit_js_eval(js_expressions=js_script, key="keyboard_listener_v3") # Updated key
