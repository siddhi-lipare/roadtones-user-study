...
RESPONSES_PATH = os.path.join(RESPONSES_DIR, 'study_results.csv')
...
def save_response(email, age, gender, video_data, caption_data, choice, study_phase, question_text, was_correct=None):
    """Saves a single response to a local CSV file."""
    ...
    response_data = {
        ...
        'sample_id': caption_data.get('caption_id') or caption_data.get('comparison_id') or caption_data.get('change_id'),
        'question_text': question_text,
        'user_choice': str(choice),
    ...
    try:
        if not os.path.isfile(RESPONSES_PATH):
            df.to_csv(RESPONSES_PATH, mode='w', header=True, index=False)
        else:
            df.to_csv(RESPONSES_PATH, mode='a', header=False, index=False)
    except Exception as e:
        st.error(f"Failed to write to local CSV file: {e}")
